...
- 🚀 **大文件处理** - 智能分块处理，支持高达 2 GB 的大文件，不会崩溃
...
> 📦 **大小限制**：单个文件最大 2 GB。由于浏览器内存限制，即使使用分块处理，所有块在最终合并前仍需保持在内存中。
...
- 🚀 **Large File Processing** - Intelligent chunked processing supports files up to 2 GB without crashes
